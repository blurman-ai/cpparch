...
| Asset | Когда | glibc |
|-------|-------|-------|
| `archcheck-<v>-linux-x86_64.tar.gz` | современные runners (`ubuntu-24.04`/`latest`), меньше размер | **нужен ≥ 2.38** |
| `archcheck-<v>-linux-x86_64-static.tar.gz` | старые / неизвестные дистрибутивы: **Astra 1.7**, Debian 10, RHEL 8 | нет (полностью статический) |

Динамический собран на `ubuntu-24.04` (`-static-libstdc++ -static-libgcc`:
libstdc++/libgcc вшиты, остаётся зависимость от glibc). Проверено: на
`ubuntu-22.04` (glibc 2.35) и старее падает с `version 'GLIBC_2.38' not found`.

Статический собран с `-static` — libc вшита, зависимости от glibc нет вообще,
...
Чтобы взять статический — поменяйте суффикс asset'а в сниппете выше:
