...
> **Совместимость (важно).** Asset собран на `ubuntu-24.04` (gcc-13,
> `-static-libstdc++ -static-libgcc` — libstdc++/libgcc вшиты, остаётся только
> зависимость от glibc). **Требуется glibc ≥ 2.38.** Проверено: на GitHub-hosted
> `ubuntu-24.04` / `ubuntu-latest` работает; на `ubuntu-22.04` (glibc 2.35) и
> старых дистрибутивах (Astra 1.7, Debian 11, RHEL 8) — **нет**, бинарь упадёт
> с `version 'GLIBC_2.38' not found`. Для таких раннеров берите fallback
> (сборка из исходников) ниже. Более широкий baseline — отдельная задача
> дистрибуции (#142 «Не делать»: package managers / отдельные сборки под старый glibc).

### Fallback / developer path: сборка из исходников

Если нужной платформы нет среди release-asset'ов, ваш runner старее glibc 2.38
(`ubuntu-22.04` и ниже), или вы хотите собрать из своего коммита:
