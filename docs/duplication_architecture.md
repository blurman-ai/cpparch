...
discovery+excludes → lex+selective-normalize → fragmentation
   ...
   → precision-фильтры (#059)                   [отсев idiom-FP]
...
### 3.6. Классификатор характера дубля (#059 P3b)
...
### 3.7. Precision-фильтры (#059) — три уровня по цене
...
- **commit / baseline = gate.** baseline морозит исторические дубли; CI валит PR
  ...
  упирается в гейт и выносит общее. Confirm (#059) нужен здесь, чтобы гейт не
...
```
detect (#053/#056 recall) → confirm (#059 precision) → ДЕЙСТВИЕ
