...
### 9.1. Интерфейс + тонкие делегирующие реализации — ⚠️ НЕ ПРОВЕРЕНО (#116)

Гипотеза поведения (рассуждение из #109, 2026-06-12): типовой полиморфный
паттерн «интерфейс + N похожих реализаций, каждая делегирует своему бэкенду
одной-двумя строками» НЕ должен репортиться как копипаст, тремя эшелонами:

1. **Порог размера**: тело `{ backend_.glDraw(c, state_); }` ≈ 10 токенов <
   `minTokens = 30` (fragmenter.h) — не становится фрагментом вовсе.
2. **Selective normalization**: callee-имена сохраняются — реализации,
   делегирующие РАЗНЫМ бэкендам (`glDraw` vs `vkDraw`), различаются именно в
   сохранённых токенах → скоринг ниже гейта.
3. **Extractability test** (правила классификации FP): одинаковые СОДЕРЖАТЕЛЬНЫЕ
   тела — честный TP (можно вынести общий хелпер).

Смоук на 2 файлах подтвердил только эшелон 1 (делегаты не фрагментируются), но
корпус из 2 фрагментов вырожден для относительного rare-df — эшелоны 2–3 на нём
не проверяемы. **Направленных тестов нет — поведение считать неподтверждённым
до закрытия #116** (интерфейс из 10 методов с разной arity, реализации в 1–2
строки; после зелёных тестов переписать этот раздел как проверенный).
