...
> **Замечание про таксономии.** В архитектурной спеке используется отдельная
> классификация правил по источнику архитектурной истины:
> `intrinsic / repo-inferred / user-declared`. Это **другая** таксономия.
> Чтобы не путать читателя, в этом каталоге используются мнемонические
> **буквы по бэкенду**: `P / S / G / R / D / X`.
...
- **Эмпирика, а не источники правил** (отдельный жанр в этом каталоге): [code_clones.md](code_clones.md) — таксономия клонов + gap-анализ; [dedup_techniques_corpus.md](dedup_techniques_corpus.md) — реальные дедуп-рефакторинги в OSS как ground truth для TP; [ai_drift_cases.md](ai_drift_cases.md) — AI-коммиты как тест-кейсы DRIFT-правил.
- [ai_smell_signals.md](ai_smell_signals.md) — таксономия наблюдаемых AI-smell сигналов: что считать AI-assisted accretion, какие признаки сильные, какие слабые и какие из них вообще стоит переводить в правила `archcheck`.
- [goodfirstissue_dogfood.md](goodfirstissue_dogfood.md) — dogfood archcheck по 10 живым C++-проектам с goodfirstissue.dev: вскрытые дефекты тула (vendor #127, SF.9 impl-idiom #126, SF.8 guard #128, extensionless #125) и реальные TP-классы. Зрелые репы как тест корректности, не demo-витрина.
