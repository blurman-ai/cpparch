...
> Версия документа: 2.4. Изменения от 2.3 (2026-06-23):
>
> - Current-state отделён от roadmap: v0.1 описан как zero-config physical-design gate
>   + advisory regression layer; YAML module-rule enforcement, libclang, SARIF,
>   suppressions и rule toggles явно помечены future.
> - Gate/advisory семантика синхронизирована с shipped CLI: check gates SF.9 only;
>   drift gates DRIFT.1/2/4.CYCLE; `--diff` gates new/grown cycles and new god-headers.
> - Технологический стек приведён к репозиторию: `ryml`, Catch2, CMake/Ninja,
...
Open source CLI для проверки архитектурных инвариантов в C++ проектах. **Shipped-ядро v0.1 — zero-config physical design gate:** archcheck строит include-граф без `compile_commands.json`, запускает консервативные правила Core Guidelines SF.* / Lakos и держит CI-gate узким: plain check падает только на SF.9 cycles, drift-baseline — на DRIFT.1/2/4.CYCLE, PR `--diff` — на new/grown cycles и new god-headers. Остальные shipped-сигналы остаются advisory и явно помечаются в text/JSON.

YAML module rules (`modules:` + `rules:` с `layers` / `independence` / `forbidden`) уже парсятся и валидируются, но **не enforce'ятся в v0.1**. Runtime enforcement модульных границ — headline v0.2 ([ADR-001](decisions/001-config-rules-deferred-to-v0.2.md)). Для legacy-проектов есть `--baseline`: фриз текущих finding-ов, дальше отчёт показывает только новые.
...
Набор default-правил имеет явную атрибуцию: циклы, god-headers и include-chain по [Lakos](https://www.amazon.com/Large-Scale-Software-Design-John-Lakos/dp/0201633620), правила из секции SF [C++ Core Guidelines](https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines#S-source) (Stroustrup, Sutter). Future-метрики Мартина (I/A/D) остаются opt-in roadmap, не shipped CLI. Каждое default-правило цитирует признанный источник — это снимает аргумент «это твоё мнение».
...
- Сканирует исходники fast preprocessor backend-ом: без `compile_commands.json`, без libclang.
- Строит include-граф и проверяет shipped default rules:
  - **SF.9** cycles — core gate в plain check.
  - **SF.7/SF.8** — source-file hygiene advisories.
  - **Lakos.GodHeader / Lakos.ChainLength** — physical-design advisories в check-mode.
- Поддерживает `--baseline`: фриз текущих finding-ов, дальше отчёт показывает только новые.
- Поддерживает graph-baseline drift: `DRIFT.1`, `DRIFT.2`, `DRIFT.4.CYCLE` gate;
  `DRIFT.3`, `DRIFT.4.NEW`, `DRIFT.4.SDP` advisory.
- Поддерживает PR `--diff`: gate на new/grown cycles и new god-headers; structural/hygiene
  drift (`added_edges`, chain/NCCD growth, SATD, test co-evolution, local complexity,
  flag arguments, new clones) report-only.
- Принимает `.archcheck.yml` v1, валидирует `modules:` / `rules:` и применяет
  `thresholds:`; runtime enforcement module rules — v0.2.
- Выдаёт text и JSON (`--format=json`) для `check` и `--diff`.
- Возвращает non-zero exit code только при gated finding/regression (см. §Exit codes).
...
Shipped advisory-слой не превращает archcheck в линтер: эти сигналы diff-scoped,
regression-oriented и не ломают CI. Их задача — показать риск нового кода рядом с
архитектурной дельтой, а не заменить clang-tidy / bug finder / style gate.

---

## Дефолтный анализ (без конфига)

Когда archcheck запущен без `--config`, применяется набор универсальных правил с явной атрибуцией. Каждое правило цитирует признанный источник — это снимает аргумент «это твоё мнение». В v0.1 default-набор включён целиком; точечные rule toggles и inline suppressions — future surface, не shipped CLI.
...
**SF.4 (`#include` идут перед другими декларациями)** — намеренно не дефолт: проверяется тривиально, но продуктовой ценности почти нет — это ещё один style-check, а не архитектурный инвариант. Shipped CLI не имеет `--enable=SF.4`. Оценку правил см. в [`docs/research/`](research/).
...
В v0.1 shipped-механизмов `--disable=...`, `--enable=...` и inline suppression-комментариев нет.
Настраиваемая поверхность сегодня — `thresholds:` в `.archcheck.yml` и baseline-файлы.
Точечное включение/выключение правил и suppression-комментарии относятся к future config
surface и должны появляться только с отдельным CLI/JSON/test-контрактом.
...
Drift-анализ в v0.1 намеренно узкий:

- только intrinsic file-level dependency graph;
- отдельный graph baseline (`--save-graph-baseline` / `--drift-baseline`);
- gate только на `DRIFT.1`, `DRIFT.2`, `DRIFT.4.CYCLE`;
- `DRIFT.3`, `DRIFT.4.NEW`, `DRIFT.4.SDP` и pre-existing intrinsic findings — advisory;
- без repo inference и без автоматической параметризации агентом.

Его задача — ловить объективное структурное ухудшение относительно baseline, не превращая
legacy-долг в бесконечный красный CI.
...
**Shipped semantics:** в `--drift-baseline` это gating-регрессия. В PR `--diff`
обычное добавленное ребро остаётся advisory, если оно не создаёт new/grown cycle
и не превращает файл в new god-header.
...
# JSON-вывод для CI. Check-mode JSON содержит top-level gate и per-finding disposition.
archcheck --format json [path]

# С кастомным конфигом: validate .archcheck.yml v1 + применить thresholds.
...
- `0` — gated finding/regression отсутствует. Advisory findings могут быть в отчёте.
- `1` — найден gated finding/regression.
...
| SARIF выход | изменение схемы после появления SARIF | добавление полей (в рамках SARIF spec) |
...
| Набор дефолтных правил | добавление правила, ломающее ранее зелёные проекты | добавление advisory-only правила или правила с отдельным opt-in |
...
- **Конфиг:** YAML через `ryml` (FetchContent, без system install).
- **Build:** CMake + Ninja.
- **CI:** GitHub Actions.
- **Тесты:** Catch2 v3.
...
- **Fast backend (preprocessor-only)** — default и единственный shipped backend в v0.1. Работает без `compile_commands.json`, мгновенно, на любой build-системе. Покрывает critical-path правила v0.1: SF.7/SF.8/SF.9, Lakos.GodHeader, Lakos.ChainLength, graph-baseline drift and `--diff` graph signals.
- **libclang backend** — v0.2+ opt-in. Нужен для AST-проверок: SF.2 / SF.5 / SF.10 / SF.11, правила из секций C и I Core Guidelines, Martin's *Abstractness* (A).
...
**Cover-story:** запуск на чужом проекте, без `compile_commands.json` и libclang, даёт полезный
результат сразу: узкий gate на объективные структурные регрессии и advisory-слой для
остального риска.
...
- **`--baseline` с day one**: фриз текущих finding-ов; в check-mode gate остаётся SF.9.
- **`--diff` PR workflow**: new/grown cycles и new god-headers gate; structural/hygiene drift advisory.
- Text-репорт с цветным выводом в TTY + JSON-репорт (`check` and `--diff`).
- Exit codes (см. §Exit codes).
- Базовая CI на GitHub Actions для самого archcheck.

**Цель:** пользователь может склонить репо, запустить `archcheck`, получить осмысленный короткий отчёт,
заморозить legacy debt baseline-ом и встроить `--diff` в CI. Без CMake-плясок, без libclang.

### v0.2 — libclang backend + остальные SF (3–4 недели)

- **Runtime enforcement YAML-модульных правил** (`layers` / `independence` / `forbidden`).
...
Текущий release-readiness статус живёт в [`backlog/TASK_TRACKER.md`](../backlog/TASK_TRACKER.md).
В этом spec не дублируется ручной список блокеров, чтобы он не протухал.

Ближайшая продуктовая линия после v0.1:

1. Довести публичную применимость zero-config first-run на реальных репах с bundled deps
   (vendored/generated exclusion и corpus sign-off).
2. Выпустить v0.1 с shipped CLI-контрактом: check/drift/diff gate/advisory semantics,
   text/JSON output and baseline workflows.
3. Начать v0.2: runtime enforcement YAML module rules, libclang semantic backend,
   SARIF and rule-management surface.
...
*Версия 2.4. Документ перерабатывается по ходу обсуждения; конкретные изменения — см. список в шапке.*
