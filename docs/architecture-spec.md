...
### Классы правил по источнику параметров
...
#### Класс A. Zero-config / intrinsic rules
...
- можно включать по умолчанию;
- хорошо подходит для CI на первом запуске;
- правило должно быть объяснимо одной фразой;
- false positive rate должен быть низким.

#### Класс B. Repo-inferred rules

Правило требует архитектурной гипотезы, но эта гипотеза может быть
**выведена автоматически** по структуре репозитория, истории и фактическому
графу зависимостей.
...
- исторически сложившиеся “фасады” и hot spots;
- baseline текущего графа.

#### Класс C. User-declared rules
...
### Новый класс: drift-regression rules

Отдельный продуктовый инсайт для эпохи AI-assisted development:
существует полезный класс правил, которые ловят **не абсолютный плохой код**,
а **подозрительное структурное ухудшение относительно уже существующего
состояния репозитория**.

Это не “архитектурный DSL пользователя” и не “ещё один линтер”.
Это правила, которые отвечают на вопрос:

> "Какой локально-удобный, но глобально-размывающий архитектуру шаг был только
> что сделан в этом изменении?"

Именно такие правила лучше всего бьются с drift-эффектами, характерными для
AI-кодинга: shortcut-зависимостями, раздуванием графа, усилением hub-узлов,
размыванием интерфейсных границ и ростом blast radius.

#### Drift rules: первичный список

##### `DRIFT.1 no_new_shortcut_edge`

**Класс:** A (`zero-config`)

**Что ловит:** новое прямое ребро зависимости, которое “срезает угол” через уже
существующий многошаговый путь.

**Формализация:**

- строится baseline dependency graph проекта;
- для каждого нового ребра `u -> v`, появившегося в diff, проверяется,
  существовал ли до изменения путь `u => v` длиной `>= 2`;
- если такой путь существовал, новое ребро помечается как `shortcut edge`.

**Почему это важно для AI drift:** агент часто выбирает кратчайший путь к
рабочему патчу и добавляет прямую зависимость туда, где раньше граница была
сохранена через промежуточный слой.

##### `DRIFT.2 no_new_cycle_or_cycle_growth`

**Класс:** A (`zero-config`)

**Что ловит:** появление нового цикла или увеличение уже существующего strongly
connected component.

**Формализация:**

- baseline и post-change графы раскладываются на SCC;
- нарушение есть, если появился новый SCC размером `> 1`, появился self-loop,
  или существующий SCC вырос по числу узлов / рёбер.

**Почему это важно для AI drift:** цикл часто возникает не как намеренный
дизайн, а как локальная “докрутка” зависимости в обратную сторону.

##### `DRIFT.3 public_surface_growth`

**Класс:** B (`repo-inferred`)

**Что ловит:** расползание зависимостей, видимых из фактической публичной
поверхности проекта.

**Формализация:**

- header считается фактически публичным, если он включается из файлов вне
  собственного directory subtree, либо попадает в явные `include/public`
  конвенции проекта;
- для такого header считается транзитивное замыкание зависимостей до и после
  изменения;
- нарушение/предупреждение возникает при аномальном росте closure или при
  появлении нового внешнего / internal subtree в публичной поверхности.

**Почему это важно для AI drift:** агент часто “случайно” протаскивает
implementation detail в интерфейс, потому что локально это быстрее, чем строить
новую boundary abstraction.

##### `DRIFT.4 blast_radius_growth`

**Класс:** A (`zero-config`)

**Что ловит:** резкий рост числа файлов / translation units, потенциально
затронутых изменением header-а или центрального узла графа.

**Формализация:**

- для изменённых header-ов считается число достижимых снизу файлов/TU в
  dependency graph до и после изменения;
- нарушение/предупреждение возникает, если blast radius заметно вырос
  относительно baseline или резко изменил ранг файла внутри репозитория.

**Почему это важно для AI drift:** “один маленький include” может превратить
локальную правку в структурный рычаг, который тянет за собой полпроекта.

##### `DRIFT.5 hub_reinforcement`

**Класс:** A (`zero-config`)

**Что ловит:** усиление уже центральных узлов графа или превращение нового
узла в hub.

**Формализация:**

- на baseline-графе считаются `fan-in`, `fan-out`, `reachability` и/или другая
  простая centrality-метрика;
- если PR добавляет новые зависимости в узел, уже находящийся в верхнем хвосте
  распределения, либо сам создаёт новый outlier, это `hub reinforcement`.

**Почему это важно для AI drift:** агент любит тянуться к уже существующим
`common`, `util`, `manager`, `base` точкам, усиливая их центральность вместо
создания локальной, ограниченной абстракции.

##### `DRIFT.6 boundary_bypass_of_existing_entrypoint`

**Класс:** B (`repo-inferred`)

**Что ловит:** обход фактически существующего фасада / entrypoint между двумя
подсистемами.

**Формализация:**

- по baseline-графу ищутся исторически доминирующие точки входа из subtree `A`
  в subtree `B`;
- если почти все cross-subtree зависимости шли через малое множество файлов
  `S`, а новое изменение добавляет прямую зависимость `A -> b`, где `b ∈ B`,
  но `b ∉ S`, это `boundary bypass`.

**Почему это важно для AI drift:** агент использует “внутреннюю кишку” чужой
подсистемы вместо уже сложившейся интеграционной точки.

##### `DRIFT.7 scattered_new_boundary_access`

**Класс:** B (`repo-inferred`)

**Что ловит:** размазывание новой внешней зависимости по нескольким точкам
сразу вместо локализации интеграции.

**Формализация:**

- если один и тот же новый target subtree / header начинает использоваться из
  нескольких файлов в разных поддеревьях в рамках одного изменения;
- и при этом не появился новый общий adapter / facade file,
  это `scattered boundary access`.

**Почему это важно для AI drift:** агент часто интегрирует “по месту”, не
создавая отдельную точку соединения.

##### `DRIFT.8 hotspot_inflow`

**Класс:** B (`repo-inferred`, history-aware)

**Что ловит:** приток новых зависимостей в уже горячие архитектурные hotspots.

**Формализация:**

- по git history считается churn для файлов / поддеревьев;
- если новое изменение добавляет зависимости из спокойных зон в уже горячие
  узлы или подсистемы, это `hotspot inflow`.

**Почему это важно для AI drift:** агент естественным образом тянется к месту,
где “уже есть логика”, даже если это место давно перегружено.

### Практическое разделение для продукта

На уровне UX и CI эти классы правил должны жить по-разному:

- **Класс A (`zero-config`)**
  можно запускать сразу на любом репозитории;
- **Класс B (`repo-inferred`)**
  AI-агент предлагает конфиг и объяснение, пользователь подтверждает или правит;
- **Класс C (`user-declared`)**
  появляются только после явного описания архитектурного контракта.

Рекомендуемый onboarding:

1. Запуск zero-config правил.
2. Построение baseline и drift-regression отчёта.
3. AI-assisted inference вероятных модулей / фасадов / boundaries.
4. Подтверждение inferred-конфига пользователем.
5. Переход к user-declared architecture policy rules.
