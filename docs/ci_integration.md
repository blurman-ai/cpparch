...
материализует оба состояния, считает архитектурную дельту и отдельно помечает
gating-регрессии и advisory-сигналы.
...
- `0` — gated-регрессии нет. Advisory-сигналы могут быть в отчёте.
- `1` — gated-регрессия: new/grown cycle или new god-header. CI должен fail-нуть.
- `2` — git/IO/config ошибка, невалидный revspec, не git-репо.
...
Добавленные рёбра, рост chain/NCCD, SATD, test co-evolution, local complexity,
flag-argument drift и new-clone drift сейчас advisory: они видны в отчёте, но
не ломают job.
...
[#018](../backlog/completed/018_crt_git_diff_analysis.md). Полная реализация:
...
### Почему shallow base-fetch

`archcheck --diff origin/<base>..HEAD` делает только две git-операции:
`git diff --name-only A B` (сравнение **двух деревьев**) и `git worktree add
--detach <ref>` (checkout **одного дерева**). Ни `rev-list`, ни `merge-base`, ни
обхода диапазона `A..B`: `parseRevspec` просто режет строку по `..` на два рефа.
Значит нужен **только base-снапшот** (один коммит + дерево), а не история.

`actions/checkout@v4` по умолчанию делает shallow clone (`--depth=1`) — HEAD есть,
а `origin/<base>` нет. Дотягиваем его **одним** снапшотом:
`git fetch --no-tags --depth=1 origin "+refs/heads/<base>:refs/remotes/origin/<base>"`.
Проверено на shallow-песочнице: в локальном графе 2 коммита, `merge-base`
недоступен, `archcheck --diff` строит реальный baseline-граф и отдаёт корректный
вердикт. На больших репах это экономит десятки секунд/сотни МБ против
`fetch-depth: 0`.

`fetch-depth: 0` (вся история) остаётся **простым, но тяжёлым** fallback'ом, если
не хочется отдельного fetch-шага. Эквивалент через явные SHA (например для
merge-стратегий, где base/head известны из payload):
...
В `merge_group` `HEAD` — это уже сгенерированный merge-commit очереди; base берётся
из `github.event.merge_group.base_ref` (в этом событии `github.base_ref` пуст), а
revspec `origin/<base>..HEAD` остаётся корректным.

## Edge-cases

| Сценарий                                | Поведение                                              |
|-----------------------------------------|--------------------------------------------------------|
| Shallow clone, baseline-ref не резолвится | ⚠️ НЕ exit 2: warning в stderr + сравнение с пустым деревом (всё «добавлено») → возможен ложный gate. Дофетчите base. Ужесточение — [#144](../backlog/new/144_min_diff_unresolved_baseline_exit2.md) |
| Force-push в PR-ветку                   | OK — сравниваем по revspec, прошлый прогон не нужен    |
| Merge-commit как HEAD                   | OK — worktree берёт snapshot-tree, не diff с родителями. Покрытие тестом — [#022](../backlog/completed/022_min_diff_merge_commit_coverage.md) |
| PR не трогает C/C++                     | fast-path: exit 0 и пустой отчёт без построения графов. См. [#023](../backlog/completed/023_maj_diff_skip_when_no_cpp_changes.md) |
| Rebase/squash на стороне PR             | без разницы — снапшоты, не история                     |
| `compile_commands.json` отсутствует     | для shipped-правил (SF.7/8/9, Lakos, drift) **не нужен** — include-scanner backend. Семантические правила v0.2+ потребуют его в обоих worktree |
...
`stdout` — текстовый отчёт или JSON (`--format=json`). Text показывает
gating-блок и advisory-блоки, затем итоговую строку `gate: ok|fail`. JSON
имеет стабильную схему `version: 1`, top-level `gate`, `gating` и `advisory`.
См. [regression_report.h](../include/archcheck/diff/regression_report.h),
...
SARIF — future/v0.2+ канал (см. [docs/architecture-spec.md](architecture-spec.md)
§«Roadmap»). JSON уже shipped для `check` и `--diff`.
...
- [#018](../backlog/completed/018_crt_git_diff_analysis.md) — реализация `--diff` (завершено).
- [#022](../backlog/completed/022_min_diff_merge_commit_coverage.md) — тест на PR с merge-commit-ом (завершено).
- [#023](../backlog/completed/023_maj_diff_skip_when_no_cpp_changes.md) — fast-path при отсутствии C/C++ изменений (завершено).
- [#025](../backlog/completed/025_maj_pr_comment_integration.md) — публикация отчёта комментарием на PR (завершено).
