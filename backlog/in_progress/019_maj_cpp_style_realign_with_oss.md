...
- **2026-05-27** — step 3/3 group 3/4: rename 5 методов `DependencyGraph`: `add_node` → `addNode`, `add_edge` → `addEdge`, `has_edge` → `hasEdge`, `node_count` → `nodeCount`, `path_of` → `pathOf`. Через sed-batched (Serena перегружен после предыдущих rename'ов). 0 identifier residue. Build OK, 115/115 tests PASSED, lizard 0 warnings, dogfood идентичен.
- **2026-05-27** — step 3/3 group 4/4: rename 7 struct-полей: `raw_token` → `rawToken`, `source_file` → `sourceFile`, `exact_path_index` → `exactPathIndex`, `suffix_index` → `suffixIndex`, `baseline_size` → `baselineSize`, `current_size` → `currentSize`, `path_to_id_` → `pathToId_` (приватное поле DependencyGraph, trailing underscore сохранён). Через sed. 0 residue. Build OK, 115/115 tests PASSED, lizard 0 warnings, dogfood идентичен. **Step 3/3 завершён.**
