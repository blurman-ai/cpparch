...
- Перед детекцией циклов **слить** узлы с одинаковым (dir + stem), где один —
  impl-маркер, в один логический компонент (Лакос). Рёбра внутри компонента
  выкидываются, межкомпонентные — остаются.
