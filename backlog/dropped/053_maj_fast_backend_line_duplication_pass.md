...
## Итог
**Статус:** superseded by #056 (решение 2026-06-01, см. docs/duplication_architecture.md, шапка)

Line-based детектор был реализован и работал (Type-1/verbatim + Type-2-lite),
но архитектурным решением подсистема сведена к одному токеновому детектору #056:
он покрывает Type-1 как частный случай Type-3, а канонический провал line-подхода
(копия с переименованием = ~12% совпадения строк против 100% по токенам) устранён
нормализацией. Код #053 удалён из дерева (остался в git-истории).

**Что пережило задачу (архив знаний):**
- FP-логика перенесена в лексер #056: схлопывание raw-string блобов `R"(...)"` в
  один `lit`, выброс мёртвых `#if 0`/`#if false` блоков.
- Метрика `line`/`normLines` в Pair — иллюстративное наследие line-подхода
  (и сегодня — рабочая нога P0.6 joint-floor).
- Эмпирика sweep-ов и маски экзотики — в research-доках подсистемы.

Все пункты «Следующие шаги» выше написаны ДО решения 2026-06-01 и им отменены:
порт line_dup_scanner в продукт не состоится. Их живые наследники: baseline/gate
для дублей — план §7 duplication_architecture.md; precision — #083.
**Дата закрытия:** 2026-06-11
