...
- [ ] Расширить `example_archcheck_pr.yml`:
  - [ ] Триггеры `pull_request` + `merge_group` с одним и тем же job-name (для merge queue).
  - [ ] Step: capture archcheck output в файл + tee в stdout.
  - [ ] Step: append `archcheck-diff.txt` в `$GITHUB_STEP_SUMMARY` через `cat archcheck-diff.txt >> "$GITHUB_STEP_SUMMARY"`.
  - [ ] Step: post sticky-comment через `marocchino/sticky-pull-request-comment@v2` с `header: archcheck-diff` и `recreate: false`. Skip на `merge_group` event (там нет PR).
  - [ ] `if: always()` на comment-step, чтобы регрессии (exit 1) тоже попадали в комментарий.
  - [ ] `permissions: pull-requests: write` на job-уровне (минимум для sticky-comment).
- [ ] Обновить шапку example_workflow с пояснением, какие три канала показа отчёта в нём задействованы.
- [ ] В [docs/ci_integration.md](docs/ci_integration.md) §«Каналы публикации в PR» — указать прямой `uses:` snippet (а не только название action).
- [ ] Smoke: создать PR с заведомой регрессией в этом репо (archcheck dogfoods свой workflow), убедиться что появляется sticky-comment.
