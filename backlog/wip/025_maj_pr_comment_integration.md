...
- [~] Smoke: PR `smoke/025-sticky-comment-test` открыт; первый CI-прогон упал на сборке (GCC 13 build issue); фикс запушен, второй прогон в процессе.
...
- Ждём результата CI на smoke PR: проверяем появление sticky-comment в conversation-tab.

## Следующие шаги

1. Убедиться что sticky-comment появился → закрыть PR без merge, удалить `fixtures/smoke_025/`.
2. Если sticky-comment получится слишком длинным (много рёбер), добавить truncation в `archcheck` (`--diff --top N`) — отдельная задача.
