...
## Статус-чек 2026-06-11 (актуализация списка «Осталось»)

1. ~~FP-гарды из #070~~ ✅ сделано (P0.1–P0.6, P0.9, P1.1/1.2/1.4 в
   duplication_scanner.cpp; P1.3 удалён как no-op 1329d06; P0.4 удалён как
   TP-killer 1fbc9f4; P0.7/P0.8 удалены 2026-06-03).
2. Reporters: text ✅ (CLI --duplication); **JSON для pairs — НЕ сделан** (остаток).
3. Фикстуры: ✅ частично — fp_corpus_eval (r2.tsv, #084) + PMD known-answer (#106,
   2ff4522); размеченный корпус-фикстуры в tests есть.
4. Dogfood: ✅ self-scan работает (`--duplication src` — 1 известная пара
   toLowerCopy → #108); CI-гейта на дубли нет (за пределами v0.1 — advisory).

**Остаток задачи фактически один: JSON-вывод duplication pairs.** Решить: нужен ли
он в v0.1 (advisory-режим) или уехать в v0.2 вместе с gate-режимом (§7 arch-дока).
