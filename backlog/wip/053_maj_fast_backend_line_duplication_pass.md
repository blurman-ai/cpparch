...
**Заблокирован:** —
**Related:** #006 (spec_refactor — fast backend как часть двух-бекендной схемы), #033 (ai_drift_dataset — duplication как сильный AI-drift сигнал), #052 (cross_tu_duplication_detector — AST-слой, точный и дорогой; **комплементарен** этой задаче), #054 (ai_repo_duplication_run — потребитель сигнала), #055 (dogfood_json_escape_dedup — наш собственный helper-дубль `jsonEscape`, найденный этим проходом, см. §«Эталон находок»)
...
- [experiments/line_duplication/main.cpp](../../experiments/line_duplication/main.cpp) — рабочий standalone C++20 порт (~1160 стр.), не в основном билде.
- [experiments/line_duplication/PERF.md](../../experiments/line_duplication/PERF.md) — time + peak RSS на референс-деревьях.
- [experiments/line_duplication/LOCAL_SWEEP_REPORT.md](../../experiments/line_duplication/LOCAL_SWEEP_REPORT.md) — исторический raw-sweep по ~20 репам.
- [experiments/line_duplication/PROJECT_EXAMPLES_REPORT.md](../../experiments/line_duplication/PROJECT_EXAMPLES_REPORT.md) — актуальный app-only прогон + `examples/`.
...
| **Разошедшиеся (gapped)** | **высокая** | Type-3 — никто из нашего (см. Non-goals) |
...
- **Полный Type-3** (gapped-копии ВНЕ одноимённых файлов — разошедшийся блок
  inline в середине чужой функции) — это edit-distance по токенам, long-term,
  НЕ в line-проход. file-twin rate ловит только gapped в одноимённых файлах.
