...
**Статус:** done (продуктовая precision ≈86–91% получена; `DRIFT.NEW_CLONE` #123 зашиплен)
...
> **Статус 2026-06-23:** product objective закрыт: precision продуктового `DRIFT.NEW_CLONE`
> получена (≈86–91%), full 185-repo run отменён как не меняющий решение. Остаток — board
> hygiene: перенести/оформить задачу в `completed/`, не dev blocker.
...
Продуктовая цель достигнута: токеновый new-clone-detector в `--diff` зашиплен как
`DRIFT.NEW_CLONE` (#123, в релизе 0.1.0), а его precision измерена на корпусе —
**≈86–91%** (triage 70 находок). Полный 185-реп прогон отменён как не несущий решения
(precision уже защитима для advisory-first). Задача отыграла роль корпусной валидации
порогов и закрывается; дальнейшая precision/recall-валидация гейта ведётся в #124.

- **Как работает:** per-commit клон-замер на fast-бэкенде, baseline-снимок + diff-скоуп
  (клон ≥N токенов, ≥1 инстанс целиком в diff).
- **Чем управляется:** пороги токенов/precision из corpus-triage; продуктовый вход — `--diff`.
- **С чем связана:** прекурсор #123 (shipped `DRIFT.NEW_CLONE`); валидация — #124; корпус — #131.
- **Диагностика:** `archcheck --diff` на коммите с копипастом → строка `new clone drift (advisory)`.
