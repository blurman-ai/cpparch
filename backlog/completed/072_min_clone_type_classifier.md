...
- [ ] `fixtures_clone_type/` — пары EXACT / RENAMED / LITERAL / STRUCTURAL в одном файле, сверка меток в выводе спайка (спайк-уровень; продуктовые `fixtures/clone_type/pass|fail_*` заводятся при интеграции #056)

## Как работает

Каждый токен несёт `raw` (исходный текст) рядом с нормализованным `sym`; пара фрагментов
выравнивается по нормализованной последовательности. **LD.10** ставит метку из сравнения
выровненных токенов: совпали и `seq`, и `raw` → `EXACT`; разошёлся только `raw`-идентификатор
→ `RENAMED`; разошёлся только литерал → `LITERAL`; разошлась нормализованная `seq` →
`STRUCTURAL`; смесь → `MIXED`. **LD.11** (`explainPair`) показывает matched-токены, similarity %
и список спрятанных нормализацией переименований/смен литералов — разработчик видит характер
клона и *почему* он совпал, не читая оба фрагмента глазами.

## Итог

**Статус:** completed (research-спайк). LD.10 + LD.11 реализованы и проверены на фикстуре в
`experiments/partial_duplication/`. Продуктовая интеграция (перенос в бинарь + настоящие
`fixtures/clone_type/`) сознательно отложена до порта #056 — отдельным шагом, не блокером.
