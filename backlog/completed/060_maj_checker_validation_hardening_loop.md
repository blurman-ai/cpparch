...
## Итог
**Статус:** completed — валидационная петля отработана полностью (iter1–4):
- iter1: глазная разметка 200 пар → precision --diff 16.5%, таксономия FP
  (coincidental 46% / other-renames 17% / segmentation 15%) → VALIDATION_ITER1.md;
- iter2: дешёвые фиксы #061 (-M) + #064 (vendor) — закоммичены (0beb2f7);
- iter3–4: 4 surface-дискриминатора опровергнуты на размеченных данных ДО
  ре-верификации (отрицательный результат — тоже результат, записан);
  код #056 откатан к чистому состоянию.
Наследники: предложения по классам FP → #070; precision-потолок токенов → #083.
Сегодняшний пост-скриптум (#107): глазная методология петли переросла в
disagreement-triage по внешним оракулам.
