...
_2026-06-02; ссылки на папки задач синхронизированы 2026-06-11 (точечно, без пересмотра приоритетов). Известный долг трекера: дубли номеров #071 (new + future) и #072 (wip + completed); #079 фактически завершена, но лежит в wip — разруливать через /backlog-review._
...
| Security hardening | `new/#105` | модель угроз «CI на недоверенном репо» (S3–S6 аудита 2026-06-11); S1/S2 уже закрыты кодом — желательно до любого публичного релиза |
...
| Post-audit cleanup | `new/#104` | мёртвый код / дубли / lizard-долг из аудита 2026-06-11; не блокирует MVP |

## OUT — вне MVP v1

### Duplication / clone-detection ветка

- `wip/#053`, `wip/#054`, `wip/#056`, `wip/#060`, `wip/#061`, `wip/#070`, `wip/#072`, `wip/#083` (blocked)
- `new/#062`, `new/#064`, `new/#088`
- `completed/#063`
...
- Продуктовые кандидаты, advisory/delta-first: `new/#093` (flag-argument), `new/#094`
  (param accretion), `new/#095` (config-bag), `new/#096` (SATD delta), `new/#097`
  (test co-evolution), `new/#098` (god-file growth), `new/#099` (indentation proxy —
  fallback/absorbed by #101), `new/#100` (defect attractor), `new/#101` (local
  complexity drift).
...
Не блокеры MVP v1; порядок внутри волны: #093 (shared text/signature scan) →
#094/#095/#099/#101; #096 (shared git_exec/diff_query) → #097/#098/#100.
