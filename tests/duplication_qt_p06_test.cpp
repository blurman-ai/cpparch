...
TEST_CASE("P0.6 verification: does it actually filter low line-overlap pairs?",
          "[duplication][p0.6][guards]") {
...
void dummy5() {
    ...
    std::vector<std::pair<std::string, std::string>> filesDiffFiles = {
        {"data1.cpp", block1},
        {"data2.cpp", block2},
        {"dummy1.cpp", dummyBlocks[0]},
        {"dummy2.cpp", dummyBlocks[1]},
        {"dummy3.cpp", dummyBlocks[2]},
        {"dummy4.cpp", dummyBlocks[3]},
    ...
    SECTION("P0.6 disabled - identical blocks in DIFFERENT FILES") {
        ...
        if (!result.fragments.empty()) {
            ...
            for (size_t i = 0; i < result.fragments.size(); i++) {
                ...
                std::cout << "  [" << i << "] tokens=" << f.tokenCount
...
void onReplaceClick() {
    ...
    if (pEditor) {
        ...
        std::vector<std::pair<std::string, std::string>> qtFiles = {
            {"slots.cpp", qtSlot1 + "\n\n" + qtSlot2}
        };
